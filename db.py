...
class Database:
    ...
    def sessionID(self, id: str) -> str:
        ...
        self.cursor.execute("SELECT username FROM Login WHERE sessionID=?", (id,))
        response = self.cursor.fetchall()
    ...
    def get_todolists(self, username: str):
        self.cursor.execute("SELECT * FROM ToDoList WHERE creator=?", (username,))
        return self.cursor.fetchall()

    def get_todolist(self, id: int):
        self.cursor.execute("SELECT * FROM Task WHERE ToDoListID=?", (id,))
        tasks = self.cursor.fetchall()
        self.cursor.execute("SELECT * FROM TodoList WHERE ID=?", (id,))
    ...
    def update_task(self, taskID, task_json):
        ...
        self.cursor.execute("UPDATE ToDoList SET name=? WHERE ID=?", (name, taskID))

        # Remove existing tasks in Task table
        self.cursor.execute("DELETE FROM Task WHERE ToDoListID=?", (taskID,))

        # Create new tasks in Task table
        values_list = []
        values_string = ", ".join([f"({taskID},?,?)"] * len(tasks))
        values_string = (
            """INSERT INTO Task (ToDoListID,name,description)
                         VALUES """
            + values_string
        )
    ...
    def get_user(self, sessionID: str) -> str:
        self.cursor.execute(
            "SELECT username FROM Login WHERE sessionID=?", (sessionID,)
        )
        response = self.cursor.fetchall()
    ...
    def create_todolist(self, sessionID: str):
        ...
        self.cursor.execute(
            "INSERT INTO ToDoList (creator,name) VALUES (?,?)", (username, "untitled")
        )
